...
namespace vca
{
  ...
  namespace icf
  {
    ...
    class VCA_API FeaturePipeGradients1Luv : public Featurepipe
    {
    public:
      ...
      */
      ...
      FeaturePipeGradients1Luv(const FeaturepipeType& t)
      {
        quantDir_.cfg(t);
        height_ = width_ = 0;
        border_ = 0;
      ...
      void calcMap(const cv::Mat& src) override
      {
        ...
        inputImg.convertTo(inputImgFloat, CV_32FC3, 1. / 255.);
        vector<cv::Mat> splitedInput;
        cv::split(inputImgFloat, splitedInput);
        
        sassert(splitedInput.size() == 3);

#pragma omp parallel sections
        {
#pragma omp section
          {
            cv::Mat s0(splitedInput[0].size(), CV_32FC1);
            cv::Mat s1(splitedInput[1].size(), CV_32FC1);
            cv::Mat s2(splitedInput[2].size(), CV_32FC1);
            convTri::convTri1(splitedInput[0].ptr<float>(), s0.ptr<float>(), splitedInput[0].cols, splitedInput[0].rows, 1.f);
            convTri::convTri1(splitedInput[1].ptr<float>(), s1.ptr<float>(), splitedInput[1].cols, splitedInput[1].rows, 1.f);
            convTri::convTri1(splitedInput[2].ptr<float>(), s2.ptr<float>(), splitedInput[2].cols, splitedInput[2].rows, 1.f);
