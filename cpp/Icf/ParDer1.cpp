...
namespace vca
{
  ...
  */
  ...
  void icf::ParDer1::calcMapFloat(const cv::Mat& src)
  {
...
#pragma omp parallel for
    for (int y = 1; y < static_cast<int>(src.rows - 1); y++)
    {
      ...
      {
        ptrDx[offset] = static_cast<float>(bgrCen[x + 1]) - static_cast<float>(bgrCen[x - 1]);

        ptrDy[offset] = static_cast<float>(bgrUp[x + 0]) - static_cast<float>(bgrDown[x + 0]);
      }
    }
  }

  void icf::ParDer1::calcMap(const cv::Mat& src)
  {
